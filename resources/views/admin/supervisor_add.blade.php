...
@section('content')
@php
    $activeTab = request()->query('active_tab', 'rough');
@endphp
        ...
        <div class="card-body">
            <ul class="nav nav-tabs" id="entryTabs" role="tablist">
                @foreach(['rough' => 'Rough Super', 'finish' => 'Finish Super', 'engineer' => 'Engineer', 'manager' => 'Project Manager'] as $id => $label)
                    <li class="nav-item" role="presentation">
                        <a class="nav-link @if($activeTab == $id) active @endif"
                           id="{{ $id }}-tab"
                           href="?active_tab={{ $id }}"
                           role="tab">
                           {{ $label }}
            ...
            <div class="tab-content pt-4" id="entryTabsContent">
                ...
                @foreach($tables as $id => $config)
                    <div class="tab-pane fade @if($activeTab == $id) show active @endif"
                                ...
                                @foreach($config['items'] as $item)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('supervisor.delete', ['model' => $config['title'], 'id' => $item->id]) }}"
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <ul class="pagination mb-0">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link"
                                               href="{{ $config['items']->previousPageUrl() }}&active_tab={{ $id }}"
                                    ...
                                    @if ($config['items']->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link"
                                               href="{{ $config['items']->nextPageUrl() }}&active_tab={{ $id }}"
